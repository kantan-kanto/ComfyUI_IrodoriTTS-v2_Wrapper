...
class DACVAECodec:
    ...
    def load(
        ...
        if not Path(location).exists():
            if "/" in location and not location.endswith(".pth"):
                try:
                ...
                raise FileNotFoundError(f"Irodori-TTS-v2 codec weights not found: {location}")
    ...
    def _normalize_loudness(
        ...
        try:
            ...
            loudness = torchaudio.functional.loudness(
                wav.unsqueeze(0).unsqueeze(0), int(sample_rate)
            )
            if loudness.ndim == 0:
                loudness = loudness.unsqueeze(0)
            gain_db = (float(target_db) - loudness).clamp(min=-80.0, max=80.0)
            gain = torch.pow(
                torch.tensor(10.0, device=wav.device, dtype=wav.dtype),
                gain_db / 20.0,
            ).reshape(())
